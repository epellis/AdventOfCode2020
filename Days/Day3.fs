...
module Day3 = 
    ...
    let parseInput lines = 
        ...
        {
            Features = features
            Dimensions = { X = (Seq.length features.[0]); Y = (Seq.length features) }
        }

    let countTrees landscape startingPoint velocityVector = 
        // printfn "Landscape: %A" landscape

        let rec countTreesInner landscape startingPoint score = 
            let currentFeature = landscape.Features.[startingPoint.Y].[startingPoint.X]
            let updatedScore = score + featureToScore.[currentFeature]

            // printfn "Evaluating %A -> %A" startingPoint currentFeature

            let updatedPoint = addPoints startingPoint velocityVector
            let updatedPointWithOverflow = { updatedPoint with X = updatedPoint.X % landscape.Dimensions.X } 

            if updatedPointWithOverflow.Y >= landscape.Dimensions.Y then
                updatedScore
            else
